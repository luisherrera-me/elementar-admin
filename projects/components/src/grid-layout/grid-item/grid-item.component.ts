...
import { GRID_DRAG_HANDLE, GridDragHandle } from '../directives/drag-handle';
import { GRID_ITEM_PLACEHOLDER, GridItemPlaceholder } from '../directives/placeholder';
import { GRID_RESIZE_HANDLE, GridResizeHandle } from '../directives/resize-handle';
import { GRID_ITEM_GET_RENDER_DATA_TOKEN, GridItemRenderDataTokenType } from '../grid.definitions';
import { GridService } from '../grid.service';
import { outsideZone } from '../utils/operators';
import { IsMouseEventOrMousePointerEvent, PointerClient, PointerDown, PointerUp } from '../utils/pointer.utils';

@Component({
    standalone: true,
    selector: 'emr-grid-item',
...
export class GridItemComponent implements OnInit, OnDestroy, AfterContentInit {
    ...
    @ContentChildren(GRID_DRAG_HANDLE, {descendants: true}) _dragHandles: QueryList<GridDragHandle>;
    @ContentChildren(GRID_RESIZE_HANDLE, {descendants: true}) _resizeHandles: QueryList<GridResizeHandle>;
    @ViewChild('resizeElem', {static: true, read: ElementRef}) resizeElem: ElementRef;

    /** Template ref for placeholder */
    @ContentChild(GRID_ITEM_PLACEHOLDER) placeholder: GridItemPlaceholder;
    ...
    constructor(public elementRef: ElementRef,
                ...
                @Inject(GRID_ITEM_GET_RENDER_DATA_TOKEN) private getItemRenderData: GridItemRenderDataTokenType) {
    ...
    ngOnInit() {
      ...
      console.log(gridItemRenderData);
    ...
    private _dragStart$(): Observable<MouseEvent | TouchEvent> {
        return merge(
            ...
            this._draggable$.pipe(
                switchMap((draggable) => {
                    ...
                    return this._dragHandles.changes.pipe(
                        ...
                        switchMap((dragHandles: QueryList<GridDragHandle>) => {
                            return iif(
                                () => dragHandles.length > 0,
                                merge(...dragHandles.toArray().map(dragHandle => PointerDown(dragHandle.element.nativeElement))),
                                PointerDown(this.elementRef.nativeElement)
                            )
                        })
                    );
                })
            ...
            exhaustMap(startEvent => {
                ...
                if (IsMouseEventOrMousePointerEvent(startEvent)) {
                    startEvent.preventDefault();
                }

                const startPointer = PointerClient(startEvent);
                return this.gridService.mouseOrTouchMove$(document).pipe(
                    takeUntil(PointerUp(document)),
                    outsideZone(this.ngZone),
                    filter((moveEvent) => {
                        moveEvent.preventDefault();
                        const movePointer = PointerClient(moveEvent);
            ...
            switchMap((resizable) => {
                if (!resizable) {
                    ...
                    return this._resizeHandles.changes.pipe(
                        ...
                        switchMap((resizeHandles: QueryList<GridResizeHandle>) => {
                            if (resizeHandles.length > 0) {
                                // Side effect to hide the resizeElem if there are resize handles.
                                this.renderer.setStyle(this.resizeElem.nativeElement, 'display', 'none');
                                return merge(...resizeHandles.toArray().map(resizeHandle => PointerDown(resizeHandle.element.nativeElement)));
                            } else {
                                this.renderer.setStyle(this.resizeElem.nativeElement, 'display', 'block');
                                return PointerDown(this.resizeElem.nativeElement);
                            }
                        }),
                        tap((startEvent) => {
                            if (IsMouseEventOrMousePointerEvent(startEvent)) {

...
import {
    DraggingData, GridCfg, GridCompactType, GridItemRect, GridItemRenderData, GridLayout, GridLayoutItem
} from '../grid.definitions';
import { PointerClientX, PointerClientY } from './pointer.utils';
import { Dictionary } from '../types';
import { GridItemComponent } from '../grid-item/grid-item.component';

/** Tracks items by id. This function is mean to be used in conjunction with the ngFor that renders the 'emr-grid-items' */
...
export function GetGridItemRowHeight(layout: GridLayout, gridHeight: number, gap: number): number {
 ...
 */
export function GridCompact(layout: GridLayout, compactType: GridCompactType, cols: number): GridLayout {
...
export function GetGridLayoutDiff(gridLayoutA: GridLayoutItem[], gridLayoutB: GridLayoutItem[]): Dictionary<{ change: 'move' | 'resize' | 'moveresize' }> {
 ...
 */
export function GridItemDragging(gridItem: GridItemComponent, config: GridCfg, compactionType: CompactType, draggingData: DraggingData): { layout: GridLayoutItem[]; draggedItemPos: GridItemRect } {
    ...
    const clientStartX = PointerClientX(pointerDownEvent);
    const clientStartY = PointerClientY(pointerDownEvent);
    const clientX = PointerClientX(pointerDragEvent);
    const clientY = PointerClientY(pointerDragEvent);
    ...
    const rowHeightInPixels = config.rowHeight === 'fit'
        ? GetGridItemRowHeight(config.layout, config.height ?? gridElemClientRect.height, config.gap)
        : config.rowHeight;
 ...
 * @param draggingData contains all the information about the drag
 */
export function GridItemResizing(gridItem: GridItemComponent, config: GridCfg, compactionType: CompactType, draggingData: DraggingData): { layout: GridLayoutItem[]; draggedItemPos: GridItemRect } {
    const {pointerDownEvent, pointerDragEvent, gridElemClientRect, dragElemClientRect, scrollDifference} = draggingData;
    const gridItemId = gridItem.id;
...
function getDimensionToShrink(layoutItem: any, lastShrunk: any): 'w' | 'h' {
    if (layoutItem.h <= 1) {
        return 'w';
    }
    if (layoutItem.w <= 1) {
...
export function GridItemLayoutItemAreEqual(item1: GridLayoutItem, item2: GridLayoutItem): boolean {

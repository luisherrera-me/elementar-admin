...
import { getMutableClientRect, ClientRect } from './client-rect';
import { noEmit } from './operators';
...
export function GetScrollTotalRelativeDifference$(scrollableParent: HTMLElement | Document): Observable<{ top: number, left: number }> {
    ...
    return fromEvent(scrollableParent, 'scroll', NormalizePassiveListenerOptions({capture: true}) as AddEventListenerOptions).pipe(
        map(() => {
            let newTop: number;
            let newLeft: number;
